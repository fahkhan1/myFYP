...
const DELETE_EMPLOYEE = () => {
   ...
   // const [EMPID, setEMPID] = useState("");
    ...
    const DeleteUser =async(id)=>{
       // alert(id);
       try{
       const response= await fetch(`http://192.168.100.149/Tracking/api/employee/DeleteEmployee?id=${id}`,{
        method: 'Get',
      } );
    ...
    renderItem={({item,})=>{
      ...
      return(
        <View style={styles.textStyle} >
            <Text style={{fontSize:20,}} >EmployeeID:  {item.EmployeeId}</Text>
          <Text style={{fontSize:20}}>Name:  {item.Name}</Text>
          <Text style={{fontSize:20}}>CNIC:  {item.Cnic}</Text>
          <Text style={{fontSize:20}}>PhoneNo#:  {item.PhoneNumber}</Text>
          <Text style={{fontSize:20}}>DrivingLicense:  {item.DrivingLicense}</Text>
          ...
          }} onPress={()=>DeleteUser(item.EmployeeId)}>
